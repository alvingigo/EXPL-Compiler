...
int GetFieldAddress(struct tnode* t){
    ...
    if (t==NULL){
        ...
        r1=GetFieldAddress(t->left);
        r2=GetFieldAddress(t->right);
        fprintf(intermediate,"MOV R%d,[R%d]\n",r1,r1);
        fprintf(intermediate,"ADD R%d,R%d\n",r1,r2);
        freeReg();
        
    }
    else if (t->nodetype==N_VAR){
        if (t->Lentry!=NULL){
            r1=getReg();
            ...
            field=Flookup(t->varname,prevtype->fields);
...
int GetMemoryAddress(struct tnode* t){
    ...
    if (t->nodetype==N_VAR){
        ...
        printf("%d %s ",t->Gentry->binding,t->varname);
        fprintf(intermediate, "MOV R%d, %d\n", r1, t->Gentry->binding);
        return r1;
...
int codeGen(struct tnode *t){
    ...
    switch (t->nodetype){
        ...
        case N_INIT:
            ...
            fprintf(intermediate, "MOV R0,\"Init\"\n");
            fprintf(intermediate, "PUSH R0\n"); // function code Initiate"
